...
FUNDAMENTALS_PATH = Path(__file__).with_name("fundamental_dashboard.html")
INDICATORS_PATH = Path(__file__).with_name("indicators_table.html")
...
@app.get("/fundamental_dashboard.html")
def fundamentals():
    """Serve the fundamental dashboard generated by --fundamental-dashboard.

    Not regenerated on request: it is refreshed by --update-fundamentals /
    --fundamental-dashboard runs (manual, cron, or the earnings checker).
    """
    if not FUNDAMENTALS_PATH.exists():
        return (
            "Fundamental dashboard not available — run "
            "bin/python main.py --fundamental-dashboard first.",
            404,
        )
    return send_file(FUNDAMENTALS_PATH)


@app.get("/indicators")
@app.get("/indicators.html")
@app.get("/indicators_table.html")
def indicators_table():
    """Serve the generated technical-indicator confluence report."""
    if not INDICATORS_PATH.exists():
        return (
            "Technical indicators report not available — run "
            "bin/python main.py --indicators-table first.",
            404,
        )
